...
export const MovieCard = ({ movie }) => {
    ...
    const moviePoster = poster_path ? `https://image.tmdb.org/t/p/w500${poster_path}` : fallbackImage;
    
    return (
    <div className="max-w-40 sm:max-w-72 bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700">
        <Link to={`/movie/${id}`}>
            <img className="rounded-t-lg" src={moviePoster} alt={original_title} />
        </Link> 
        <div className="p-5">
            <Link to={`/movie/${id}`}>
